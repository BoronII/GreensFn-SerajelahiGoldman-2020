...
// This is the header file where all user functions are declared.
#include "GreensFn.h"
...
g0method is an integer used to choose the method for calculating g0.
...
//-----------------------------------------------------------------------------/
    ...
    // In the code ntissparams will be the first iterator (rows), and nsp will be the second iterator (columns).
    ...
    // The next two variables are used to calculated the cutoff distance after which the influence of a source can be ignored.
    PO2max = 70; // The maximum anticipated PO2 field (mmHg)
    cutoff_dist = PO2max*fac/diff[1]/0.15;
    
    if (greensverbose) {
        printf("cutoff_dist = %f (microns) \n", cutoff_dist);
    }
    //-----------------------------------------------------------------------------/
    //-----------------------------------------------------------------------------/
    ...
    //-----------------------------------------------------------------------------/
    //-----------------------------------------------------------------------------/
    // Analyse the network (analyzenet() in the working source);
    ...
    //-----------------------------------------------------------------------------/
    //-----------------------------------------------------------------------------/
    // Setting up more arrays (setuparrays2(nnv, nnt) in the working source);
    ...
    // the following two lines appear right after setuparrays2 in the working source
    for (inod = 1; inod <= nnod; inod++) nodvar[inod] = nodname[inod];
    
    //-----------------------------------------------------------------------------/
    //-----------------------------------------------------------------------------/
    // This next part is for visualization (nodvar, segvar, "Current\\NetNodesSegs.ps") in the working source);

    //fname[80] = "netNodesSegs.ps"; Removed this line and just put the string into fopen directly because there were problems opening the file.
    xmin = 0.;
    ...
    //-----------------------------------------------------------------------------/
    //-----------------------------------------------------------------------------/
    // The main calculation (greens() in the working source);
    //-----------------------------------------------------------------------------/
    ...
    for (i = 1; i <= nnv; i++) {
        ...
        for (j = 1; j <= nnv; j++) {
            ...
            else if (dist > cutoff_dist) {// added Jan 27 to check if idea for approximation is useful - Baran
                gvv[i][j] = 0;
            }
            else {
    ...
    for (jx = 1; jx <= mxx; jx++) for (jy = 1; jy <= myy; jy++) for (jz = 1; jz <= mzz; jz++) {
        ...
        if (jx*jy*jz != 1) {
            if (dist > cutoff_dist) dtt[jx][jy][jz] = 0; // Added Jan 27th for approximation purposes - Baran
            else if (is2d) dtt[jx][jy][jz] = fac * 2. / w2d * log(r2d / dist);
    ...
    // declaration of vt, size_vt, tv, size_tv. vt is a 2d array that holds, for each vessel point, the indices of all the tissue points that are inside the cutoff distance set near the begining of the code. size_vt is a vector that holds for each vessel points the number of tissue tissue points that are inside the cutoff distance - Baran
    // size_vt = vector(1, nnv);
    size_vt = (int *) calloc(nnv+1, sizeof(int ));
    for(i=1;i<=nnv;i++)
        size_vt[i] = -4;
    ...
    //vt = imatrix(1, nnv , 1, nnt);
    vt = (int **) calloc(nnv+1, sizeof(int *));
    for(i=1;i<=nnv;i++) {
        vt[i] = (int *) calloc(nnt+1, sizeof(int ));
    }
    for(i=1;i<=nnv;i++)
        for(j=1;j<=nnt;j++)
            vt[i][j] = -6;
    ...
    //size_tv = vector(1, nnt);
    size_tv = (int *) calloc(nnt+1, sizeof(int ));
    for(i=1;i<=nnt;i++)
        size_tv[i] = -7;
    ...
    //tv = imatrix(1, nnt , 1, nnv);
    tv = (int **) calloc(nnt+1, sizeof(int *));
    for(i=1;i<=nnt;i++) {
        tv[i] = (int *) calloc(nnv+1, sizeof(int ));
    }
    for(i=1;i<=nnt;i++)
        for(j=1;j<=nnv;j++)
            tv[i][j] = -87;
    ...
    // This next part is used in the approximation for gvt - Baran Feb 2020
    ...
    if(DEBG==1) {
        for(j=1;j<=nnv;j++) for(i=1;i<=size_vt[j];i++) printf("%d %d %d %d %d\n", nnv,j,size_vt[j], i, vt[j][i]);
    }
    // This next part is used in the approximation for gtv - Baran Feb 2020
    for (itp = 1; itp <= nnt; itp++) {
        mm = 0;
        for (i = 1; i <= nnv; i++) {
            dist = sqrt(SQR(ax[1][i] - axt[tisspoints[1][itp]])
    ...
    if(DEBG==1) {
        for(j=1;j<=nnt;j++) for(i=1;i<=size_tv[j];i++) printf("%d %d %d %d %d\n", nnt,j,size_tv[j], i, tv[j][i]);
    }
    /****************************************************************************
    ...
    *******************************************************************************/
    ...
    for (kmain = 1; kmain <= nmax; kmain++) {
        ...
        for (i = 1; i <= nnv; i++) {
            ...
            for (itp = 1; itp <= size_vt[i]; itp++) {
                dist = sqrt(SQR(ax[1][i] - axt[tisspoints[1][vt[i][itp]]])
                            + SQR(ax[2][i] - ayt[tisspoints[2][vt[i][itp]]])
                            + SQR(ax[3][i] - azt[tisspoints[3][vt[i][itp]]]));
                if (dist <= req) {
    ...
    vesselconv:;
        ...
        for (itp = 1; itp <= nnt; itp++) {
            ...
            for (i = 1; i <= size_tv[itp]; i++) {
                dist = sqrt(SQR(ax[1][tv[itp][i]] - axt[tisspoints[1][itp]])
                            + SQR(ax[2][tv[itp][i]] - ayt[tisspoints[2][itp]]) + SQR(ax[3][tv[itp][i]] - azt[tisspoints[3][itp]]));
                if (dist <= req) {
...
void blood(float c, float h, float *p, float *pp)
{
    ...
    if (c < chigh) {// It looks like alphab*p (the amount of oxygen dissolved in the blood) is ignored when the concentration is higher than clow. - BS
        if (c / h / cs < 1.) {
